...
**Currently building:** Phase 6, item-by-item, in this order: (1) i18n (English/Hindi/Marathi) — ✅ done, (2) multilingual semantic double-check via Cloudflare Workers AI (BGE-M3) — ✅ done (originally shipped on a self-hosted Hugging Face Space, then migrated to Cloudflare Workers AI — see that row's changelog), (3) equipment image upload — ✅ done, (4) geo distance filtering — ✅ done, (5) real availability_slots calendar — ✅ done, (6) PWA install/offline — ✅ done, (9) AI-parser smart routing + follow-up questions — ✅ done (see table; §6.1/§4.4 updated to match). TypeScript and Zustand from the original §2 stack are intentionally NOT being adopted — plain JS/JSX and React Context are staying as-is; this is a deliberate, permanent decision, not a pending item.
...
| 2 | Equipment CRUD (owner side) | ✅ `AddEquipment.jsx` (create/edit) + "My Listings" tab in `Profile.jsx` (list/pause/delete). Former deviation (closed in Phase 6 item 3): photo upload now live — public `equipment-photos` bucket, client-side compression to ~200KB, up to 4 photos per listing; illustrated art remains only as the fallback when a listing has no photos. |
| 2 | Rules-based filter (hard compatibility) | ✅ `src/lib/rulesFilter.js` implements §6.3 (equipment_type, operation, crop, HP range, availability, excludes own listings). Former deviation (closed in Phase 6 item 4): geo distance filtering is now live — PostGIS `nearby_equipment` RPC enforces both the farmer's search radius and `service_area_radius_km`. Still runs client-side, unchanged since Phase 2 — Phase 4 only replaced the *score* it computes, not the filter itself (see Phase 4 ranking row below). |
...
| 3 | Frontend free-text input wired into Phase 2 filter | ✅ New "freetext" step at the top of `DescribeJob.jsx` (`src/lib/llmClient.js` calls the backend) — pre-fills crop/operation/land, farmer reviews/edits in the existing wizard steps, then flows into the same `runRulesFilter` (§6.3) as before. Skipping the free-text step or any LLM failure drops straight into the unchanged manual wizard. (Extended in Phase 6 item 9: the parser now also extracts place/date and the frontend smart-routes — full parses skip straight to the review step instead of walking the whole wizard.) |
...
| 3 | §4.2/§2: equipment photo upload — Supabase Storage bucket never created; listings show illustrated art instead of real photos. | **Fix.** Create a public Supabase Storage bucket (1GB free), compress client-side to ~150–200KB before upload (per §2's original note), wire into `AddEquipment.jsx`. | ✅ Done. Public `equipment-photos` bucket (2MB/file cap, JPG/PNG/WebP) with storage RLS: anyone reads, owners write only under `equipment/<own-uid>/`. `src/lib/imageUpload.js` compresses client-side (1600px long edge → quality search → ~150–200KB, EXIF stripped) and uploads with typed errors; `PhotoPicker.jsx` stages up to 4 photos with per-photo status/size; `AddEquipment.jsx` uploads first, then writes the row once (orphan cleanup if the row write fails; removed photos deleted only after a successful save). `EquipmentPhoto`/`EquipmentGallery` show real photos in Recommendations/EquipmentDetails/Profile/MyBookings/Booking with illustrated art as the automatic fallback. Deleting a listing best-effort deletes its photo folder. Verified live: 3.7MB → 283KB upload round-trip + public read (E2E test, cleaned up). |
| 4 | §6.3/§6.4: geo distance filtering/ranking — Mapbox/PostGIS never wired up. `service_area_radius_km` not enforced; `distance` ranking feature is a constant placeholder. | **Fix, free-tier path.** Supabase Postgres already ships PostGIS (free, no separate service) — enable the extension, store `users.location`/`equipment.location` as `geography(Point)`, use `ST_DWithin`/`ST_Distance` directly in the existing Supabase queries. Skip Mapbox's paid geocoding API for now — take lat/lng from the browser's free Geolocation API (farmer/owner already on their device) or a manual pin-on-map (e.g. free `react-leaflet` + OpenStreetMap tiles, no API key needed) instead of address-string geocoding. | ✅ Done. PostGIS was already enabled; added `latitude`/`longitude` numeric columns on `equipment`+`users` with a trigger keeping `geography(Point)` in sync (invalid coords → null, never a wrong point), GIST indexes, and a `nearby_equipment` RPC enforcing BOTH the farmer's search radius (clamped 0.5–500 km) and each listing's `service_area_radius_km`, nearest-first via KNN (migration: `supabase/migrations/20260910_phase6_geo.sql`, mirrored in `schema.sql`). Owners pin locations via lazy-loaded `LocationPicker.jsx` (react-leaflet + OSM, geolocate button, manual lat/lng fallback); farmers capture location in DescribeJob (mocked-E2E-verified) with a 10/25/50/100 km radius picker, coords reused from the profile next visit. Unlocated listings still appear (distance unknown) so the marketplace works during migration; any RPC failure falls back to the plain list (§4.5 pattern). `distance_km` flows into the heuristic's proximity nudge + reason line, a "Nearest first" sort, and the LightGBM `distance` feature (normalized against 50 km, null → neutral 0.5). Verified live E2E: farmer 3.5 km from a pinned listing sees exactly it with "3.5 km away" (test data cleaned up). |
| 5 | §5.2: `availability_slots` table — never built; booking conflicts checked via ad-hoc date-overlap query against `bookings` instead. | **Fix.** Build the real `availability_slots` table per §5.2, replace the ad-hoc overlap query in `EquipmentDetails.jsx`/`Booking.jsx` with slot lookups. No new infra — same Supabase Postgres. | ✅ Done. Migration `supabase/migrations/20260911_phase6_slots.sql` (mirrored in `schema.sql`): valid-range CHECK, (equipment,start,end) index, backfill of the live Confirmed/In Use bookings, owner-manage policy (USING + WITH CHECK) plus a farmer self-release policy for cancelled locks. `src/lib/availability.js` (slot CRUD, `checkAvailability` with always-enforced booked clashes + unconstrained fallback for owners who never set windows, localized range formatting); owner `AvailabilityManager.jsx` on the edit-listing page (add/remove windows, booked locks read-only); farmers see offered windows on `EquipmentDetails.jsx`. Slot lookups replaced the ad-hoc overlap queries in `EquipmentDetails.jsx` (request), `Booking.jsx` (confirm pre-check + date lock with rollback on failure, release on cancel/complete), and `DescribeJob.jsx` (`availability_quality` ranking signal). Verified live E2E 13/13 against the real project (overlap + outside-window blocks, lock write + release through RLS); test data cleaned up. |
| 6 | §2: `vite-plugin-pwa` — never installed. No manifest, no installable/offline app shell (the FCM service worker is push-only, not a full PWA). | **Fix.** `npm i -D vite-plugin-pwa`, add manifest using existing `public/icons.svg` assets. Directly supports the "mobile, no registration" goal — installable home-screen app, offline caching, zero cost. | ✅ Done. Hand-rolled instead of the plugin (zero new deps, same result): `public/manifest.webmanifest` + generated PNG icons (192/512/maskable/apple-touch) + `public/sw.js` (network-first navigations with offline fallback, cache-first hashed assets, network-only APIs) + trilingual `public/offline.html`. PROD-only SW registration, `beforeinstallprompt` capture (`src/lib/pwa.js`), dismissible `InstallBanner.jsx` on the Dashboard. Verified: SW active, manifest valid, offline reload renders the app shell. |
...
| 9 | §4.4/§6.1: the free-text AI parse pre-filled crop/operation/land but always dropped the farmer into the full 6-step manual wizard — even when the message contained everything; and a message with almost nothing produced a 422 (`operation` was required) with no targeted follow-up, so the AI step felt useless. | **Fix.** Backend: also extract `location_text` + `needed_date` (prompt injects today's IST date; strict validation, see §6.1), `operation` nullable — 422 now means only "both providers unusable". Frontend (`DescribeJob.jsx`): smart routing — complete parse → review/summary step, core-only (crop+operation+land, no place/date) → location step, partial → inline follow-up questions for exactly the missing slots (crop → operation → land, quick-tap answers); review rows gain per-field edit jumps; the bottom-nav button on the AI step now parses instead of silently skipping the AI. Backward compatible both ways: new frontend tolerates old-backend responses without the new keys, old frontend ignores the new fields. | ✅ Done. Backend `backend/test_llm_service.py` 10/10 (mocked providers: new-slot pass-through, old-shape compat, date/location validation, nullable operation, still-422-when-both-down, prompt contents, response model). Frontend E2E 22/22 against a stub `/requirements/parse` on both dev and production builds — full-text → review with all values + provider note, core-only → location, follow-up Q&A incl. custom acres, Hindi questions, 422 → manual wizard, old-shape → location, review edit-jumps, zero page errors. |
...
**Farmer flow:** Enter requirement — free text (AI extracts everything it can, asks follow-up questions for exactly what's missing, then jumps to location-fetching or straight to the summary) or manual step-by-step → confirm parsed requirement → view filtered+ranked equipment → compare → select → choose date/time → book → track status → complete rental.
...
### 6.1 LLM layer
- **Providers:** Groq API (primary) → Gemini API (fallback) → manual structured form (final fallback, always available, uses same output shape). (Order flipped from the original Gemini→Groq — see the Phase 3 disclosure in §0 STATUS.)
- **Prompt design:** System prompt includes the full allowed vocabulary from `taxonomy.json` (crop list, operation list, equipment type list), today's date (IST, so relative dates resolve correctly), and instructs the model to output **only** JSON matching this exact shape, using only terms from the provided lists:
```json
{
  "crop": "string (from allowed crop list, or null)",
  "area_acres": "number or null",
  "operation": "string (from allowed operation list, or null if the message says nothing about the work)",
  "equipment_type": "string (from allowed equipment type list, or null if inferable from operation)",
  "location_text": "string (place as written, original script, or null)",
  "needed_date": "string (YYYY-MM-DD, resolved against today's date, or null)"
}
```
- If the LLM outputs a term not in the taxonomy, run it through the semantic matcher (6.2) before rejecting.
- **Validation (backend, `llm_service.py`):** `needed_date` must be a real calendar date within [today, today+730 days], otherwise null; `location_text` is stripped and capped at 120 chars, blank → null. `operation` is nullable — a partial parse is HTTP 200, not 422; 422 now means only "both providers unreachable/unusable".
- **Smart routing (frontend, `DescribeJob.jsx` — Phase 6 item 9):** a complete parse jumps straight to the review/summary step; crop+operation+land with no place/date jumps to the location step; anything less stays on the AI step and asks follow-up questions for exactly the missing core slots (crop → operation → land, quick-tap answers). Location/date keep their dedicated steps (GPS capture + calendar UI). The manual wizard remains the always-available fallback per §4.5.
