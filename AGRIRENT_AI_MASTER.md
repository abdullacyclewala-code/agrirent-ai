...
**Currently building:** Phase 6, item-by-item, in this order: (1) i18n (English/Hindi/Marathi) — ✅ done, see item 1 in the table below, (2) multilingual semantic double-check on HF Spaces — ✅ done this session, see item 2 in the table below, (3) equipment image upload — next up, (4) geo distance filtering, (5) real availability_slots calendar, (6) PWA install/offline. TypeScript and Zustand from the original §2 stack are intentionally NOT being adopted — plain JS/JSX and React Context are staying as-is; this is a deliberate, permanent decision, not a pending item.
...
2. Create a new Space (https://huggingface.co/new-space): SDK **Docker**, visibility **Public** (required on the free tier).
3. Push `hf-space-semantic/`'s contents to *that Space's own git remote* (not this GitHub repo) — exact commands in `hf-space-semantic/README.md` "Deploying this".
4. Wait for the Space to finish its first build (a few minutes — downloading the base image + ~470MB model) and show "Running".
5. (Recommended) Space Settings → Variables and secrets → add secret `API_TOKEN` with a random value, since free Spaces are public URLs.
6. Redeploy `/backend` on Render (same service as Phases 3-5) with two new env vars: `HF_SPACE_URL` (the Space's URL from step 4) and, if step 5 was done, `HF_SPACE_API_TOKEN` (same value).
Until steps 1-6 are done, nothing breaks — `semantic_match.py` keeps using its Phase 3 difflib fallback exactly as before; this is purely additive.
...
| 2 | §6.2: semantic matching fallback uses `difflib` + synonym map only (English-oriented). Doesn't help with Hindi/Marathi dialectal variance the LLM sometimes mis-maps. | **Fix, with a specific role — see §6.2a below.** Deploy `paraphrase-multilingual-MiniLM-L12-v2` (sentence-transformers, free/OSS, 50+ languages incl. Hindi/Marathi) on a free Hugging Face Space (Docker SDK, free CPU tier = 16GB RAM, fits comfortably vs Render's 512MB). This does **not** replace the LLM — see §6.2a for exact role. | ✅ Done. New standalone service `hf-space-semantic/` (FastAPI + `sentence-transformers`, one endpoint `POST /match`) — see its README for API shape, cold-start behaviour, and step-by-step deploy instructions. Backend calls it via new `backend/app/multilingual_client.py`, wired into `backend/app/semantic_match.py`'s fallback chain as step 3 of 4 (exact match → synonym map → **HF Space embedding call** → difflib last resort), matching §6.2a's "double-check on every non-exact LLM field, not just total failure" framing — see that file's updated docstring for the exact reasoning on why exact/synonym-resolved terms skip the network call. Degrades safely: `HF_SPACE_URL` unset (or the call failing/timing out) falls straight through to the pre-existing difflib path — verified with unit-level tests covering exact match, synonym match, a successful mocked HF response, a mocked "no match above threshold" response, and a mocked timeout, all in this session (see PR notes / commit). **Manual step still required (per this row's own decision — cannot be done from a repo commit alone):** create a free Hugging Face account, create a Space from `hf-space-semantic/`, and set `HF_SPACE_URL` (+ optional `HF_SPACE_API_TOKEN`) in Render — full steps in `hf-space-semantic/README.md` "Deploying this" and `backend/README.md` "Multilingual semantic double-check". Until that's done, this phase item is code-complete but not yet *live* — the app keeps working exactly as before (difflib fallback) in the meantime. |
...
| Semantic matching | Exact synonym-map lookup → **`paraphrase-multilingual-MiniLM-L12-v2`** (sentence-transformers, hosted on a free Hugging Face Space, `hf-space-semantic/`) as an LLM double-check/corrector across English/Hindi/Marathi, not just an outage fallback — see §6.2a → `difflib` lexical similarity as last resort |
...
| Hosting | Frontend → Vercel · Backend → Render (free tier) · Semantic-match model → Hugging Face Spaces (free CPU tier), code in `hf-space-semantic/`, **manual deploy step required — see that folder's README** · DB/Auth/Storage → Supabase |
